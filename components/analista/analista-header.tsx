...
interface AnalistaHeaderProps {
  sidebarCollapsed?: boolean
}

export default function AnalistaHeader({ sidebarCollapsed = false }: AnalistaHeaderProps) {
    ...
    async function getUserInfo() {
      ...
      if (adminUsuario) {
        try {
          ...
          setUserName(usuario.nome || usuario.email || null)
        } catch (error) {
          console.error("Erro ao parsear dados do usuário:", error)
        }
      }

      // Se não encontrou no localStorage, extrair do email
      ...
      try {
        ...
        setTenantName('Contratandoplanos')
      }
    }
    getUserInfo()
  }, [router, userName])
  ...
  return (
    <header 
      className={`analista-header h-16 px-2 sm:px-3 md:px-4 flex items-center justify-between fixed top-0 z-20 shadow-lg transition-all duration-300 ${
    ...
    >
      ...
      <div className="flex items-center h-full">
        <Link href="/analista" className="text-sm sm:text-base md:text-lg font-semibold text-[#0F172A] block md:hidden hover:opacity-90 transition-opacity">
          Portal<span className="hidden sm:inline"> Analista</span>
        </Link>
        <Link href="/analista" className="text-base md:text-lg font-semibold text-[#0F172A] hidden md:block hover:opacity-90 transition-opacity">
          {tenantName || 'Contratandoplanos'} <span className="text-gray-600 font-normal">Analista</span>
        </Link>
      </div>
