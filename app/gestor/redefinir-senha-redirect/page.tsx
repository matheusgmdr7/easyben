...
/**
 * Página intermediária para redirecionamento de redefinição de senha de gestor
 */
export default function RedefinirSenhaGestorRedirectPage() {
  ...
  useEffect(() => {
    const processarRedirecionamento = async () => {
      try {
        ...
        const redirectUrl = `${protocol}//${hostname}/gestor/redefinir-senha?access_token=${encodeURIComponent(accessToken)}&refresh_token=${encodeURIComponent(refreshToken || "")}&type=${type}`
