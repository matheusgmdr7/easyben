...
export default function RedefinirSenhaCorretorPage() {
  ...
  useEffect(() => {
    ...
    if (errorCode === "otp_expired" || (errorDesc && errorDesc.toLowerCase().includes("expired"))) {
      setErro(
        "Este link expirou. Os links de recuperação são válidos por 1 hora e servem para um único uso. " +
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Usar tokens armazenados ao abrir o link (o hash pode ser consumido pelo Supabase depois)
        ...
        <div className="w-full max-w-md">
          <div className="bg-white rounded-lg shadow-md p-6 text-center">
            ...
            <div className="flex flex-col gap-3">
              <Link href="/corretor/recuperar-senha">
                <Button className="w-full bg-[#0F172A] hover:bg-[#1E293B]">
                  Solicitar nova redefinição de senha
                </Button>
              </Link>
              <Link href="/corretor/login" className="text-[#0F172A] hover:underline text-sm font-medium">
                Voltar para o login
              </Link>
            </div>
          </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <h1 className="text-2xl md:text-3xl font-bold text-[#0F172A] mb-2">Redefinir Senha</h1>
