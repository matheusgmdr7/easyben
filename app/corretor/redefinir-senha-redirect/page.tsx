...
/**
 * Página intermediária para redirecionamento de redefinição de senha de corretor
 * Detecta o tenant e redireciona para o domínio correto
 */
export default function RedefinirSenhaCorretorRedirectPage() {
  ...
  useEffect(() => {
    const processarRedirecionamento = async () => {
      try {
        ...
        // Construir URL de redirecionamento para o mesmo domínio (encode para evitar quebra em tokens)
        const redirectUrl = `${protocol}//${hostname}/corretor/redefinir-senha?access_token=${encodeURIComponent(accessToken)}&refresh_token=${encodeURIComponent(refreshToken || "")}&type=${type}`
