...
/**
 * Página intermediária para redirecionamento de redefinição de senha de analista
 */
export default function RedefinirSenhaAnalistaRedirectPage() {
  ...
  useEffect(() => {
    const processarRedirecionamento = async () => {
      try {
        ...
        const redirectUrl = `${protocol}//${hostname}/analista/redefinir-senha?access_token=${encodeURIComponent(accessToken)}&refresh_token=${encodeURIComponent(refreshToken || "")}&type=${type}`
