...
export async function GET(request: NextRequest) {
  try {
    ...
    const denominadorPercentual = clienteIds.length > 0 ? clienteIds.length : Math.max(totalVidasAtivas, 1)
    ...
    return NextResponse.json({
      ...
      mes_fim: mesFim,
      criterio:
        "Titulares do grupo: IDs de clientes administradora vinculados (mesmo critério de boletos do grupo). " +
        "Fatura atrasada: status = atrasada e data de vencimento no mês indicado (para a série mensal) ou no intervalo (resumo do período). " +
        "Percentual = titulares distintos com ≥1 fatura atrasada / total de titulares com vínculo ao grupo; se não houver vínculos, usa vidas ativas como referência aproximada.",
      totais: {
