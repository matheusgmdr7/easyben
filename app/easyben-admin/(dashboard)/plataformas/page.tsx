...
export default function PlataformasPage() {
  ...
  const loadTenants = async () => {
    try {
      ...
      toast.error(`Erro ao carregar plataformas: ${error.message}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeletar = async (tenant: Tenant) => {
    if (!confirm(`Tem certeza que deseja desativar a plataforma "${tenant.nome}"?`)) {
      return
    }

    try {
      await deletarTenant(tenant.id)
      toast.success('Plataforma desativada com sucesso!')
      loadTenants()
    } catch (error: any) {
      toast.error(`Erro ao desativar plataforma: ${error.message}`)
    }
  }
  ...
  const handleAtivar = async (tenant: Tenant) => {
    try {
      ...
      toast.error(`Erro ao ativar plataforma: ${error.message}`)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Carregando plataformas...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Gestão de Plataformas</h1>
          <p className="text-gray-600 mt-1">
            Administre e configure todas as plataformas dos seus clientes
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Total de Plataformas</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Plataformas Inativas</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de Plataformas</CardTitle>
          <CardDescription>
            {tenantsFiltrados.length} plataforma(s) encontrada(s)
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          asChild
                        >
                          <Link href={`/easyben-admin/plataformas/${tenant.id}`} className="inline-flex">
