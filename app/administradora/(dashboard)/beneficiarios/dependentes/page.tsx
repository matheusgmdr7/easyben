...
export default function BeneficiariosDependentesPage() {
  ...
  async function exportarExcel() {
    try {
      ...
      const headers = ["Dependente", "CPF Dependente", "Titular", "CPF Titular", "Grupo", "Parentesco", "Idade", "Status"]
      const body = resultados.map((r: any) => [
        r.nome || "-",
        formatarCpf(r.cpf),
        r.titular_nome || "-",
        formatarCpf(r.titular_cpf),
        r.grupo_nome || "-",
        r.parentesco || "-",
      ...
      ws["!cols"] = [{ wch: 34 }, { wch: 16 }, { wch: 34 }, { wch: 16 }, { wch: 30 }, { wch: 16 }, { wch: 10 }, { wch: 14 }]
      ...
      for (let r = 1; r <= body.length; r++) {
        ...
        for (let c = 0; c < headers.length; c++) {
          ...
          ;(cell as any).s = {
            ...
            alignment: { vertical: "top", horizontal: c === 1 || c === 3 ? "center" : "left", wrapText: true },
          }
        }
      }
  ...
  async function exportarPdf() {
    try {
      ...
      const colWidths = [58, 24, 58, 24, 54, 24, 12, 14]
      const headers = ["Dependente", "CPF Dep.", "Titular", "CPF Tit.", "Grupo", "Parentesco", "Idade", "Status"]
      const rowH = 6
      let y = 12

      doc.setFontSize(12)
      doc.setTextColor(15, 23, 42)
      doc.text("Relatório de Dependentes", margem, y)
      y += 5
      doc.setFontSize(8)
      ...
      resultados.forEach((r: any, idx: number) => {
        ...
        doc.setFontSize(7)
        const valores = [
          String(r.nome || "-").slice(0, 30),
          formatarCpf(r.cpf),
          String(r.titular_nome || "-").slice(0, 30),
          formatarCpf(r.titular_cpf),
          String(r.grupo_nome || "-").slice(0, 30),
          String(r.parentesco || "-").slice(0, 12),
